...
namespace PerpustakaanAppMVC.View
{
    public partial class FrmMahasiswa : Form
    {
        ...
        private void TambahkanKeListView(Mahasiswa mhs)
        {
            var noUrut = lvwMahasiswa.Items.Count + 1;
            var item = new ListViewItem(noUrut.ToString())
            {
                SubItems = { mhs.Npm, mhs.Nama, mhs.Angkatan }
            };
            lvwMahasiswa.Items.Add(item);
        }
        ...
        private void txtNama_TextChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(txtNama.Text))
            {
                ...
                var filteredMahasiswa = listOfMahasiswa
                    .Where(mhs => mhs.Nama.IndexOf(txtNama.Text, StringComparison.OrdinalIgnoreCase) >= 0)
                    .ToList();

                foreach (var mhs in filteredMahasiswa)
                {
                    TambahkanKeListView(mhs);
                }
            }
            else
            {
        ...
        private void btnCari_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(txtNama.Text))
            {
                var foundMahasiswa = listOfMahasiswa
                    .Where(mhs => mhs.Nama.IndexOf(txtNama.Text, StringComparison.OrdinalIgnoreCase) >= 0)
                    .ToList();

                if (foundMahasiswa.Count > 0)
                {
                    lvwMahasiswa.Items.Clear();
                    foreach (var mhs in foundMahasiswa)
                    {
                        TambahkanKeListView(mhs);
                    }
                }
            ...
            {
                MessageBox.Show("Masukkan nama untuk mencari data.", "Peringatan", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
